...
public class D31NeuralControler : MonoBehaviour
{
    ...
    //* YOUR CODE HERE*//
    float fitness = distanceTravelled + ((distanceToBall.Average() * -1)*100) + ((distanceToAdversaryGoal.Average() * -1)*50) + distancefromBallToMyGoal.Average() + (distancefromBallToAdversaryGoal.Average() * -1) + (hitTheBall * 100) + (GoalsOnAdversaryGoal * 500) + ((GoalsOnMyGoal * -1) * 200) + ((hitTheWall * -1) * 10);
        return fitness;
    }
    ...
    public float GetScoreRed()
    {
        ...
        //* YOUR CODE HERE*//
        float fitness = ((GoalsOnMyGoal * -1) * 1000) + (100 * distancefromBallToMyGoal.Average()) + ((distanceToBall.Average() * -1) * 1000) + distanceTravelled + (100 * hitTheBall);
        return fitness;
    }
